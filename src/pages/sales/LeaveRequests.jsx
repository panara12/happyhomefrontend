            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2 mb-3">
                  ...
                  <span className={`px-2 py-1 rounded-full text-xs ${getLeaveTypeBadge(request.leaveType)}`}>
                    {request.leaveType}
                  </span>
                ...
                <div className="space-y-1 text-sm text-gray-600">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium">({request.days} {request.days === 1 ? 'day' : 'days'})</span>
                  </div>
                  ...
                  {request.status !== 'pending' && request.approvedBy && (
                    <div className="text-gray-500">
                      {request.status === 'approved' ? 'Approved' : 'Rejected'} by {request.approvedBy} on {request.approvedAt && new Date(request.approvedAt).toLocaleString()}
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-auto p-6">
            <div className="flex items-start justify-between mb-6">
              <div>
                ...
                <div className="text-gray-600 mt-1">Submitted on {new Date(selectedRequest.createdAt).toLocaleString()}</div>
              </div>
              <button
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <span className={`inline-block px-3 py-1 rounded-full text-sm ${getLeaveTypeBadge(selectedRequest.leaveType)}`}>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="font-medium">{new Date(selectedRequest.startDate).toLocaleDateString()}</div>
                </div>
                <div className="bg-gray-50 p-4 rounded-lg">
                  <div className="text-sm text-gray-600 mb-1">End Date</div>
                  <div className="font-medium">{new Date(selectedRequest.endDate).toLocaleDateString()}</div>
                </div>
                <div className="bg-gray-50 p-4 rounded-lg">
                  <div className="text-sm text-gray-600 mb-1">Total Days</div>
                  <div className="font-medium text-amber-600">{selectedRequest.days} {selectedRequest.days === 1 ? 'day' : 'days'}</div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                {selectedRequest.status !== 'pending' && selectedRequest.approvedBy && (
                  <div className="mt-2 text-sm text-gray-600">
                    {selectedRequest.status === 'approved' ? 'Approved' : 'Rejected'} by {selectedRequest.approvedBy} on {selectedRequest.approvedAt && new Date(selectedRequest.approvedAt).toLocaleString()}
                  </div>
                )}
              </div>
            </div>
